...
type OpenAICompatModelItem = NonNullable<OpenAICompatModelsResponse["data"]>[number];
...
const VISION_MODEL_PATTERNS = [
  "vl", "vision", "vision-language",
  "qwen-vl", "qwen2-vl", "qwen2.5-vl", "qwen-vl-max",
  "qwen-2.5-vl", "qwq-32b",
  "llava", "llava-next", "llava-llama", "moondream",
  "internvl", "internvl2", "internvl3",
  "pixtral", "mistral-nemo", "mistral-vision",
  "gpt-4o", "gpt-4-turbo", "gpt-4o-mini",
  "claude-3-opus", "claude-3-sonnet", "claude-3-5",
  "gemini-1.5", "gemini-2.0", "gemini-pro-vision",
  "yi-vision", "yi-vl",
  "minimax-vision", "minimax-vl",
  "glm-4v", "glm-vision",
...
function detectImageSupport(modelId: string, modelData?: OpenAICompatModelItem): Array<"text" | "image"> {
  const lower = modelId.toLowerCase();
  if (modelData?.capabilities?.supportsVision === true) {
    return ["text", "image"];
  }
  if (Array.isArray(modelData?.architecture?.input_modalities)) {
    if (modelData.architecture.input_modalities.some((m) => typeof m === "string" && m.toLowerCase() === "image")) {
      return ["text", "image"];
    }
  }
  if (Array.isArray(modelData?.tags) && modelData.tags.includes("vision")) {
    return ["text", "image"];
  }
...
export async function discoverOpenAICompatibleLocalModels(params: {
  ...
  try {
    ...
    return models
      .map((model) => ({ id: typeof model.id === "string" ? model.id.trim() : "", raw: model }))
      ...
      .map((model) => {
        ...
        return {
          ...
          input: detectImageSupport(modelId, model.raw),
