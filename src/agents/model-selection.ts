...
export function buildAllowedModelSet(params: {
  ...
  defaultModel?: string;
  agentId?: string;
  /** When true, only return models from providers that have a configured API key */
  onlyWithApiKey?: boolean;
}): {
  allowAny: boolean;
  ...
  if (allowAny) {
    ...
    let filteredCatalog = params.catalog;
    if (params.onlyWithApiKey) {
      const providersWithApiKey = new Set<string>();
      for (const entry of params.catalog) {
        if (hasUsableCustomProviderApiKey(params.cfg, entry.provider)) {
          providersWithApiKey.add(entry.provider);
        }
      }
      filteredCatalog = params.catalog.filter((entry) => providersWithApiKey.has(entry.provider));
    }

    return {
      allowAny: true,
      allowedCatalog: filteredCatalog,
