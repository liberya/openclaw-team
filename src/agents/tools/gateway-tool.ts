...
const GatewayToolSchema = Type.Object({
  ...
  agentWorkspace: Type.Optional(Type.String()),
  agentEmoji: Type.Optional(Type.String()),
  agentAvatar: Type.Optional(Type.String()),
});
...
export function createGatewayTool(opts?: {
  ...
  return {
    ...
    ownerOnly: true,
    description:
      "Create PERSISTENT named agents the user can manage from the Agents page, restart the gateway, or apply/patch config. For persistent named agents, ALWAYS use action=agents.create with agentName (required), agentWorkspace (required), agentEmoji (optional), agentAvatar (optional). For gateway restarts use action=restart. For config edits use config.schema.lookup before config.patch. Always pass a note parameter for config write actions. For temporary subagents (one-shot tasks), use sessions_spawn instead.",
    ...
    execute: async (_toolCallId, args) => {
      ...
      if (action === "agents.create") {
        const agentName = readStringParam(params, "agentName", { required: true, label: "agentName" });
        const agentWorkspace = readStringParam(params, "agentWorkspace", { required: true, label: "agentWorkspace" });
        const agentEmoji = readStringParam(params, "agentEmoji");
        const agentAvatar = readStringParam(params, "agentAvatar");
        ...
        return jsonResult({ ok: true, result, note });
      }
