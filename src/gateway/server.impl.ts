...
import { initAuthService, getJwtSecret } from "../auth/service.js";
import { initUserDataService } from "../auth/user-data.js";
...
export async function startGatewayServer(
  ...
  const dbClient = createPostgresClient(dbConfig);
  const jwtSecret = getJwtSecret();
  initAuthService(dbClient, jwtSecret);
  initUserDataService(dbClient);
  log.info("Auth service initialized");
  log.info("User data service initialized");

  // Auto-initialize default admin account
  try {
    const { getAuthService } = await import("../auth/service.js");
    const authService = getAuthService();
    const existingAdminId = await authService.getUserIdByEmail("admin@openclaw.ai");
    if (!existingAdminId) {
      await authService.initializeDefaultAdmin("admin@openclaw.ai", "admin123");
      log.info("Default admin account created (admin@openclaw.ai / admin123)");
    } else {
      log.info("Admin account already exists, skipping initialization");
    }
  } catch (err) {
    log.warn("Failed to initialize admin account:", err);
  }
  ...
  // Development: allow no gateway token when OPENCLAW_DEV_NO_GATEWAY_TOKEN=1
  const devNoGatewayToken = process.env.OPENCLAW_DEV_NO_GATEWAY_TOKEN === "1";
  const authOverride = devNoGatewayToken ? { mode: "none" } : undefined;
  ...
  const authBootstrap = await ensureGatewayStartupAuth({
    ...
    persist: devNoGatewayToken ? false : true,
  });
