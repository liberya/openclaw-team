...
import { migrationHandlers } from "./server-methods/migration.js";
...
const JWT_AUTH_METHODS = new Set(["jwt", "api-key"]);

function getClientUserId(client: GatewayRequestOptions["client"]): string | undefined {
  const authToken = (client as any)?.connect?.authToken;
...
function authorizeGatewayMethod(method: string, client: GatewayRequestOptions["client"]) {
  ...
  const authMethod = client.connect.authMethod;

  if (authToken && (authMethod === "jwt" || authMethod === "api-key")) {
    const role = (client as any)?.userRole ?? "user";
    const scopes = client.connect.scopes ?? [];

    if (!isRoleAuthorizedForMethod({ name: role, scopes } as any, method)) {
