...
export type AuthorizeGatewayConnectParams = {
  ...
  /** Allow unauthenticated WebSocket connections for local Control UI when allowInsecureAuth is configured. */
...
export async function authorizeGatewayConnect(
  ...
  if (auth.mode === "token") {
    ...
    if (params.allowInsecureAuth && localDirect && !connectAuth?.token) {
      return { ok: true, method: "insecure-auth" };
    }
...
export async function authorizeHttpGatewayConnect(
  params: Omit<AuthorizeGatewayConnectParams, "authSurface"> & {
    jwtSecret?: string;
    verifyJwt?: (token: string, secret: string) => Promise<{ sub: string; email: string; role: string } | null>;
    verifyApiKey?: (key: string) => Promise<{ id: string; email: string; role: string } | null>;
  },
): Promise<GatewayAuthResult> {
  ...
  if (token) {
    if (token.startsWith("Bearer ")) {
      ...
      const jwtSecret = params.jwtSecret ?? process.env.OPENCLAW_JWT_SECRET;
      if (jwtSecret && params.verifyJwt) {
        const payload = await params.verifyJwt(jwtToken, jwtSecret);
        if (payload) {
          return {
            ok: true,
            method: "jwt",
    ...
    if (params.verifyApiKey && !token.startsWith("Bearer ")) {
      const user = await params.verifyApiKey(token);
      if (user) {
        return {
          ok: true,
          method: "api-key",
...
export async function authorizeWsControlUiGatewayConnect(
  ...
  const connectAuth = params.connectAuth;
  const token = connectAuth?.token;
  const allowInsecureAuth = params.allowInsecureAuth;
  const isLocalClient = params.req ? isLocalDirectRequest(params.req, params.trustedProxies ?? [], params.allowRealIpFallback === true) : false;

  if (allowInsecureAuth && isLocalClient && !token) {
    return { ok: true, method: "insecure-auth" };
  }
  ...
  if (token) {
    ...
    if (allowInsecureAuth && isLocalClient) {
      return { ok: true, method: "insecure-auth" };
    }
  }
