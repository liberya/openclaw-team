...
export const agentsHandlers: GatewayRequestHandlers = {
  "agents.list": async ({ params, respond, client }) => {
    ...
    const cfg = loadConfig();
    let result = listAgentsForGateway(cfg);
    ...
    if (!userId && params?.sessionKey && String(params.sessionKey).startsWith('agent:main:')) {
      userId = String(params.sessionKey).replace('agent:main:', '');
    }
    if (!userId) {
    ...
    if (!admin && userId) {
      try {
        ...
        const userAgents = await userDataService.listUserAgents(userId);
        const allowedAgentIds = new Set(userAgents.map(a => a.agentId));

        if (result.agents) {
          result.agents = result.agents.filter(a =>
            a.id === DEFAULT_AGENT_ID || allowedAgentIds.has(a.id)
          );
        }
      } catch {
        result.agents = [];
  ...
  "agents.create": async ({ params, respond, client }) => {
    ...
    if (userId) {
      try {
        ...
        const existing = await userDataService.getUserAgent(userId, agentId);
        if (!existing) {
          await userDataService.createUserAgent(userId, agentId, rawName, agentDir, false);
        }
      } catch {
        // Non-fatal: agent is created in config, user link will be retried on next access
      }
    }
  ...
  "agents.update": async ({ params, respond, client }) => {
    ...
    // Prevent non-admin users from modifying the main agent
    if (agentId === DEFAULT_AGENT_ID && !isAdmin(client, params)) {
      respond(false, undefined, errorShape(ErrorCodes.FORBIDDEN, "cannot modify the main agent"));
      return;
    }
  ...
  "agents.files.set": async ({ params, respond, client }) => {
    ...
    // Prevent non-admin users from modifying files in the main agent workspace
    if (agentId === DEFAULT_AGENT_ID && !isAdmin(client, params)) {
      respond(false, undefined, errorShape(ErrorCodes.FORBIDDEN, "cannot modify files in the main agent"));
      return;
    }
