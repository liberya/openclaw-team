...
async function checkUserSessionAccess(userId: string, sessionKey: string): Promise<boolean> {
  // 新格式: agent:main:${userId}
  if (sessionKey.startsWith(`agent:main:${userId}`)) {
    return true;
  }
  return false;
}
...
export const chatHandlers: GatewayRequestHandlers = {
  "chat.history": async ({ params, respond, context, client }) => {
    ...
    if (!admin && userId) {
      const hasAccess = await checkUserSessionAccess(userId, sessionKey);
      if (!hasAccess) {
        context.logGateway.debug(`chat.history: access denied for sessionKey=${sessionKey} userId=${userId}`);
        respond(true, {
          sessionKey,
          sessionId: null,
          messages: [],
  ...
  "chat.abort": async ({ params, respond, context, client }) => {
    ...
    if (!admin && userId) {
      const hasAccess = await checkUserSessionAccess(userId, rawSessionKey);
      if (!hasAccess) {
        context.logGateway.debug(`chat.abort: access denied for sessionKey=${rawSessionKey} userId=${userId}`);
        respond(false, undefined, errorShape(ErrorCodes.UNAUTHORIZED, "Access denied to this session"));
        return;
      }
    }
  ...
  "chat.send": async ({ params, respond, context, client }) => {
    ...
    if (!admin && userId) {
      const hasAccess = await checkUserSessionAccess(userId, rawSessionKey);
      if (!hasAccess) {
        context.logGateway.debug(`chat.send: access denied for sessionKey=${rawSessionKey} userId=${userId}`);
        respond(false, undefined, errorShape(ErrorCodes.UNAUTHORIZED, "Access denied to this session"));
        return;
      }
    }
        ...
        .then(() => {
          if (!agentRunStarted) {
            const btwReplies = deliveredReplies
              ...
              .filter((p): p is ReplyPayload & { btw: { question: string } } => !!(p as unknown as { btw?: unknown }).btw);
