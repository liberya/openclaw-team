...
import { getCurrentUserId, isAdmin, verifyTokenPayload } from "./auth.js";
import { parseRestartRequestParams } from "./restart-request.js";
import { getUserDataService } from "../../auth/user-data.js";
...
export const configHandlers: GatewayRequestHandlers = {
  ...
  "config.patch": async ({ params, respond, client, context }) => {
    ...
    // Prevent non-admin users from modifying the main agent config
    if (changedPaths.some((p) => p.startsWith("agents.list"))) {
      const oldMainEntry = (snapshot.config.agents?.list ?? []).find(
        (e: Record<string, unknown>) => e.id === "main",
      );
      const newMainEntry = (validated.config.agents?.list ?? []).find(
        (e: Record<string, unknown>) => e.id === "main",
      );
      if (JSON.stringify(oldMainEntry) !== JSON.stringify(newMainEntry) && !isAdmin(client, params)) {
        respond(
          false,
          undefined,
          errorShape(ErrorCodes.FORBIDDEN, "cannot modify the main agent configuration"),
        );
        return;
      }
    }
    ...
    // Sync new agents to user_agents table for data isolation
    const token = (params as Record<string, unknown>)?.token as string | undefined;
    const authToken = (client as Record<string, unknown>)?.connect !== undefined
        ? ((client as Record<string, { authToken?: string }>).connect?.authToken as string | undefined)
        : undefined;
    const tokenPayload = (() => {
        const raw = token ?? authToken;
        if (!raw) return null;
        try {
            const cleaned = raw.startsWith("Bearer ") ? raw.slice(7) : raw;
            const parts = cleaned.split('.');
            const payloadJson = parts.length >= 2 ? parts[1] : parts[0];
            return JSON.parse(Buffer.from(payloadJson, 'base64url').toString());
        } catch { return null; }
    })();
    const userId: string | undefined = tokenPayload?.sub as string | undefined;
    ...
    if (userId) {
        try {
            ...
            const existingAgents = await userDataService.listUserAgents(userId);
            const deletedIds = new Set(existingAgents.filter(a => a.isDeleted).map(a => a.agentId));
            const oldIds = new Set(
                (snapshot.config.agents?.list ?? [])
                    .map((a: Record<string, unknown>) => a.id as string)
                    .filter((id): id is string => typeof id === 'string' && id.length > 0 && !deletedIds.has(id))
            );
            const added = (validated.config.agents?.list ?? []).filter(
                (a: Record<string, unknown>) =>
                    typeof a?.id === 'string' && (a.id as string).length > 0 && !oldIds.has(a.id as string)
            );

            for (const agent of added) {
                try {
                    await userDataService.upsertUserAgent(
                        userId,
                        agent.id as string,
                        agent.name as string | undefined,
                        agent.workspace as string | undefined,
                        (agent.default as boolean) ?? false,
                    );
                    context?.logGateway?.info(
                        `[config.patch] Synced agent: ${agent.id} for user ${userId}`
                    );
                } catch (err) {
                    context?.logGateway?.warn(
                        `[config.patch] Failed to sync agent ${agent.id}: ${String(err)}`
                    );
                }
            }
        } catch (err) {
            context?.logGateway?.warn(`[config.patch] Failed to sync user_agents: ${String(err)}`);
        }
    }
