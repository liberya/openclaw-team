...
import { getUserSpecificSessionKey } from "./storage.ts";
...
import { renderUserLoginGate } from "./views/user-login-gate.ts";
import { renderOverview } from "./views/overview.ts";
import { renderUsers, renderUserFormModal, type User } from "./views/users.ts";
import { renderModels, type ModelProvider, type ProviderPreset } from "./views/models.ts";
...
export function renderApp(state: AppViewState) {
  ...
  // 1. Auto-connect to gateway if not connected
  // 2. Show user login if not authenticated
  
  if (!state.connected) {
    return html`${renderUserLoginGate(state)}`;
  }
  ...
  return html`
    ...
    >
              ...
              state.currentUser
                ? html`
                    <div class="topbar-user">
                      <span class="topbar-user__name">${state.currentUser.name || state.currentUser.email}</span>
                      <span class="topbar-user__role">${state.currentUser.role}</span>
                      <button
                        class="btn btn--icon topbar-user__logout"
                        title="Logout"
                        @click=${() => {
                          (state as any).logout();
                        }}
              ...
              <nav class="sidebar-nav">
                ${TAB_GROUPS.filter((group) => {
                  ...
                  if (group.label === "control") {
                    ...
                    return group.tabs.some(tab => ["channels", "sessions", "cron"].includes(tab));
                  }
                  if (group.label === "agent") {
                    if (isAdmin) return true;
                    return group.tabs.some(tab => ["agents", "skills"].includes(tab));
                  }
                  return true;
                }).map((group) => {
                  const isAdmin = state.currentUser?.role === "admin";
                  ...
                  if (!isAdmin) {
                    if (group.label === "control") {
                      filteredTabs = group.tabs.filter(tab => ["channels", "sessions", "cron"].includes(tab));
                    } else if (group.label === "agent") {
                      filteredTabs = group.tabs.filter(tab => ["agents", "skills"].includes(tab));
                    }
                  }
                  const isGroupCollapsed = state.settings.navGroupsCollapsed[group.label] ?? false;
                  const hasActiveTab = filteredTabs.some((tab) => tab === state.tab);
                  ...
                  return html`
                    <section class="nav-section ${!showItems ? "nav-section--collapsed" : ""}">
                      ...
                      <div class="nav-section__items">
                        ${filteredTabs.map((tab) => renderTab(state, tab, { collapsed: navCollapsed }))}
                      </div>
                    </section>
                  `;
                })}
          ...
          state.tab === "chat"
            ? renderChat({
                ...
                onAgentChange: (agentId: string) => {
                  state.sessionKey = getUserSpecificSessionKey(agentId);
                  state.chatMessages = [];
                  state.chatStream = null;
                  state.chatRunId = null;
                  state.applySettings({
            ...
            ? renderUsers({
                ...
                onRefresh: async () => {
                  ...
                  try {
                    ...
                    const result = await state.client?.request("auth.admin.users.list", {
                      limit: 20,
                      offset: (state.usersPage - 1) * 20,
                      token: token,
                    });
                    if (result && (result as any).users) {
                      state.usersList = (result as any).users;
                      state.usersTotal = (result as any).total;
                    }
                  } catch (err) {
                    state.usersError = err instanceof Error ? err.message : "Failed to load users";
                  } finally {
                    state.usersLoading = false;
                  }
                },
                onCreateUser: async (user) => {
                  state.showUserFormModal = true;
                  state.editingUser = null;
                },
                onUpdateUser: async (userId, updates) => {
                  try {
                    const token = localStorage.getItem("authToken");
                    await state.client?.request("auth.admin.users.update", { 
                      userId, 
                      ...updates,
                      token: token 
                    });
                    ...
                    state.usersError = err instanceof Error ? err.message : "Failed to update user";
                  }
                },
                onResetPassword: async (userId, newPassword) => {
                  try {
                    const token = localStorage.getItem("authToken");
                    await state.client?.request("auth.admin.users.reset-password", { 
                      userId, 
                      newPassword,
                      token: token 
                    });
                    ...
                    state.usersError = err instanceof Error ? err.message : "Failed to reset password";
                  }
                },
                onDeleteUser: async (userId) => {
                  if (confirm("Are you sure you want to delete this user?")) {
                    try {
                      const token = localStorage.getItem("authToken");
                      await state.client?.request("auth.admin.users.delete", { 
                        userId,
                        token: token 
                      });
                      state.loadUsers?.();
                    } catch (err) {
                      state.usersError = err instanceof Error ? err.message : "Failed to delete user";
                    }
                  }
                },
              })
        ...
        ${
          state.tab === "models"
            ? renderModels({
                ...
                providers: state.modelsList,
                presets: state.modelsPresets,
                error: state.modelsError,
                isAdmin: state.currentUser?.role === "admin",
                showAddForm: state.modelsShowAddForm,
                editingProvider: state.editingModel,
                addingProvider: state.modelsAddingProvider,
                updatingProvider: state.modelsUpdatingProvider,
                manualInput: state.modelsManualInput,
                manualProviderId: state.modelsManualProviderId,
                viewingProvider: state.viewingModelsProvider,
                onViewModels: (provider: any) => {
                  state.viewingModelsProvider = provider;
                },
                onCloseViewModels: () => {
                  state.viewingModelsProvider = null;
                },
                onRefresh: async () => {
                  state.modelsLoading = true;
                  try {
                    const token = localStorage.getItem("authToken");
                    const result = await state.client?.request("models.admin.list", {
                      token: token,
                    });
                    if (result && (result as any).providers) {
                      state.modelsList = (result as any).providers;
                    }
                  } catch (err) {
                    state.modelsError = err instanceof Error ? err.message : "Failed to load models";
                  } finally {
                    state.modelsLoading = false;
                  }
                },
                onAddProvider: async (providerId, apiKey, baseUrl) => {
                  state.modelsAddingProvider = true;
                  state.modelsError = null;
                  try {
                    const token = localStorage.getItem("authToken");
                    const result = await state.client?.request("models.admin.add", {
                      providerId,
                      apiKey,
                      baseUrl,
                      token: token,
                    });
                    const discoveryFailed = (result as any)?.discoveryFailed ?? false;
                    if (discoveryFailed) {
                      state.modelsManualInput = true;
                      state.modelsManualProviderId = providerId;
                      state.modelsError = (result as any)?.discoveryError ?? "Model discovery failed. Please add models manually.";
                      // Refresh list so provider shows up
                      const listResult = await state.client?.request("models.admin.list", { token: token });
                      if (listResult && (listResult as any).providers) {
                        state.modelsList = (listResult as any).providers;
                      }
                    } else {
                      state.modelsShowAddForm = false;
                      state.modelsError = null;
                      const listResult = await state.client?.request("models.admin.list", { token: token });
                      if (listResult && (listResult as any).providers) {
                        state.modelsList = (listResult as any).providers;
                    ...
                    state.modelsError = err instanceof Error ? err.message : "Failed to add provider";
                  } finally {
                    state.modelsAddingProvider = false;
                  }
                },
                onSaveManualModels: async (providerId, modelIds: string[]) => {
                  state.modelsAddingProvider = true;
                  state.modelsError = null;
                  try {
                    const token = localStorage.getItem("authToken");
                    const result = await state.client?.request("models.admin.update", {
                      providerId,
                      models: modelIds,
                      token: token,
                    });
                    if ((result as any)?.success) {
                      state.modelsShowAddForm = false;
                      state.modelsManualInput = false;
                      state.modelsManualProviderId = "";
                      state.modelsError = null;
                      const listResult = await state.client?.request("models.admin.list", { token: token });
                      if (listResult && (listResult as any).providers) {
                        state.modelsList = (listResult as any).providers;
                    ...
                    state.modelsError = err instanceof Error ? err.message : "Failed to save models";
                  } finally {
                    state.modelsAddingProvider = false;
                  }
                },
                ...
                onUpdateProvider: async (providerId, apiKey) => {
                  state.modelsUpdatingProvider = true;
                  state.modelsError = null;
                  try {
                    const token = localStorage.getItem("authToken");
                    const result = await state.client?.request("models.admin.update", {
                    ...
                    const modelCount = (result as any)?.modelCount ?? 0;
                    if (modelCount > 0) {
                      state.modelsError = null;
                    }
                    // Refresh list
                    const listResult = await state.client?.request("models.admin.list", {
                      token: token,
                    });
                    if (listResult && (listResult as any).providers) {
                      state.modelsList = (listResult as any).providers;
                    ...
                    state.modelsError = err instanceof Error ? err.message : "Failed to update provider";
                  } finally {
                    state.modelsUpdatingProvider = false;
                  }
                },
                onRemoveProvider: async (providerId) => {
                  if (!confirm(`Are you sure you want to remove ${providerId}?`)) return;
                  try {
                    const token = localStorage.getItem("authToken");
                    await state.client?.request("models.admin.remove", {
                      providerId,
                      token: token,
                    });
                    const listResult = await state.client?.request("models.admin.list", {
                    ...
                    state.modelsError = err instanceof Error ? err.message : "Failed to remove provider";
                  }
                },
                onToggleAddForm: () => {
                  state.modelsShowAddForm = !state.modelsShowAddForm;
                  if (state.modelsShowAddForm && state.modelsPresets.length === 0) {
                    // Load presets
                    state.client?.request("models.admin.presets", {}).then((result: any) => {
                      if (result?.presets) {
                        state.modelsPresets = result.presets;
                      }
                    });
                  }
                },
                onToggleEdit: (provider) => {
                  state.editingModel = provider;
                },
                onCancelEdit: () => {
                  state.editingModel = null;
                },
      ...
      ${state.showUserFormModal ? renderUserFormModal(
        state.editingUser,
        async (user) => {
          try {
            const token = localStorage.getItem("authToken");
            await state.client?.request("auth.admin.users.create", { 
              ...user,
              token: token 
            });
            state.showUserFormModal = false;
            state.editingUser = null;
            state.loadUsers?.();
          } catch (err) {
            state.usersError = err instanceof Error ? err.message : "Failed to create user";
          }
        },
        () => {
          state.showUserFormModal = false;
