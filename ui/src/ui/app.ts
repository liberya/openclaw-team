...
import type { GatewayBrowserClient, GatewayHelloOk, LoggedInUser } from "./gateway.ts";
import type { Tab } from "./navigation.ts";
import { loadSettings, setCurrentUserId, getCurrentUserId, getUserSpecificSessionKey, type UiSettings } from "./storage.ts";
...
import type { User } from "./views/users.ts";
import type { ModelProvider, ProviderPreset } from "./views/models.ts";
...
function loadCurrentUserFromStorage(): LoggedInUser | null {
  try {
...
export class OpenClawApp extends LitElement {
  ...
  @state() password = "";
  @state() email = "";
  @state() currentUser: LoggedInUser | null = loadCurrentUserFromStorage();
  ...
  @state() usersError: string | null = null;
  ...
  @state() modelsList: ModelProvider[] = [];
  @state() modelsPresets: ProviderPreset[] = [];
  @state() modelsShowAddForm = false;
  @state() modelsAddingProvider = false;
  @state() modelsUpdatingProvider = false;
  @state() editingModel: ModelProvider | null = null;
  @state() modelsManualInput = false;
  @state() modelsManualProviderId = "";
  @state() viewingModelsProvider: ModelProvider | null = null;
  ...
  protected firstUpdated() {
    ...
    if (typeof window !== "undefined") {
      ...
      const isUsersPage = path.includes("/users") || path === "/" || path === "/ui" || path === "/ui/";
      if (isUsersPage && this.currentUser?.role === "admin") {
  ...
  protected updated(changed: Map<PropertyKey, unknown>) {
    // 当切换到 chat 标签时，确保 sessionKey 是用户特定的格式，并重新加载数据
    if (changed.has("tab") && this.tab === "chat" && this.currentUser?.id) {
      const userSpecificKey = getUserSpecificSessionKey(this.sessionKey.split(':')[1] || "main");
      if (this.sessionKey !== userSpecificKey) {
        console.log("[updated] Tab changed to chat, updating sessionKey from", this.sessionKey, "to", userSpecificKey);
        this.sessionKey = userSpecificKey;
        this.applySettings({
          ...this.settings,
          sessionKey: userSpecificKey,
          lastActiveSessionKey: userSpecificKey,
        });
        // 重新加载聊天历史和 sessions 列表
        this.chatMessages = [];
        loadChatHistory(this as unknown as OpenClawApp);
        loadSessions(this as unknown as OpenClawApp, {
          activeMinutes: 60,
        });
      }
    }
  ...
  async loginWithCredentials() {
    if (!this.email || !this.password) {
      this.lastError = "Please enter email and password";
      return;
    }

    this.lastError = null;

    const needsNewConnection = !this.client || !this.connected;
    if (needsNewConnection) {
      this.client?.stop();
      this.client = null;
      const savedSettings = { ...this.settings };
      const savedSessionKey = this.sessionKey;
    ...
    try {
      ...
      if (result && (result as any).accessToken) {
        ...
        console.log("[login] Login successful, userId:", user.id, "role:", user.role);
        
        const savedSettings = { ...this.settings };
        const savedSessionKey = this.sessionKey;
        
        console.log("[login] Reconnecting WebSocket to refresh all data...");
        this.client?.stop();
        this.client = null;
        this.settings = savedSettings;
        this.sessionKey = savedSessionKey;
        ...
        if (user.role === "admin" && this.tab === "users") {
          this.loadUsers();
        }
      } else {
      ...
      this.lastError = err instanceof Error ? err.message : "Login failed";
    }
  }
  ...
  async loadUsers() {
    if (!this.client) {
      console.log("[loadUsers] client not ready, retrying...");
    ...
    try {
      const token = localStorage.getItem("authToken");
      const result = await this.client?.request("auth.admin.users.list", {
        limit: 20,
        offset: 0,
        token: token,
      });
      if (result && (result as any).users) {
        this.usersList = (result as any).users as unknown as User[];
        this.usersTotal = (result as any).total as number;
        this.usersPage = 1;
      }
    } catch (err) {
  ...
  async loadModels() {
    if (!this.client) {
      console.log("[loadModels] client not ready, retrying...");
    ...
    try {
      const token = localStorage.getItem("authToken");
      console.log("[loadModels] Loading with token:", token ? "present" : "missing");
      
      const [providersResult, presetsResult] = await Promise.all([
        this.client?.request("models.admin.list", { token: token }),
        this.client?.request("models.admin.presets", { token: token }),
      ]);
      console.log("[loadModels] Results - providers:", providersResult, "presets:", presetsResult);
      
      if (providersResult && (providersResult as any).providers) {
        this.modelsList = (providersResult as any).providers as unknown as ModelProvider[];
      }
      if (presetsResult) {
        const presetsData = (presetsResult as any).presets;
        // Handle both object and array formats
        if (Array.isArray(presetsData)) {
          this.modelsPresets = presetsData as unknown as ProviderPreset[];
        } else if (presetsData && typeof presetsData === 'object') {
          // Convert object to array
          this.modelsPresets = Object.entries(presetsData).map(([id, value]: [string, any]) => ({
            id,
            ...value
          })) as unknown as ProviderPreset[];
        }
      }
      console.log("[loadModels] Loaded - modelsList:", this.modelsList.length, "modelsPresets:", this.modelsPresets.length);
    } catch (err) {
      console.error("[loadModels] Error:", err);
      this.modelsError = err instanceof Error ? err.message : "Failed to load models";
    } finally {
      this.modelsLoading = false;
      console.log("[loadModels] Done, loading:", this.modelsLoading);
    }
  }
