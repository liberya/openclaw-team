...
export class GatewayBrowserClient {
  ...
  async loginWithCredentials(email: string, password: string): Promise<{ token: string; user: LoggedInUser }> {
    return new Promise((resolve, reject) => {
      ...
      const handleLoginMessage = (ev: MessageEvent) => {
        try {
          const data = JSON.parse(ev.data);
          if (data.id === loginRequest.id) {
            this.ws?.removeEventListener("message", handleLoginMessage);
            if (data.error) {
              reject(new Error(data.error.message || "Login failed"));
              ...
              resolve({ token: data.result.accessToken, user });
            } else {
      ...
      this.ws?.addEventListener("message", handleLoginMessage);
  ...
  private async sendConnect() {
    ...
    if (email && password) {
      try {
        const result = await this.loginWithCredentials(email, password);
        this.opts.token = result.token;
      } catch (err) {
    ...
    console.log("[sendConnect] explicitGatewayToken:", explicitGatewayToken ? "present" : "missing");
    console.log("[sendConnect] this.opts.token:", this.opts.token ? "present" : "missing");
    ...
    if (isSecureContext) {
      ...
      console.log("[sendConnect] Before selectConnectAuth, explicitGatewayToken:", explicitGatewayToken);
      selectedAuth = this.selectConnectAuth({
        role,
        deviceId: deviceIdentity.deviceId,
      });
      console.log("[sendConnect] After selectConnectAuth, selectedAuth.authToken:", selectedAuth.authToken);
      if (this.pendingDeviceTokenRetry && selectedAuth.authDeviceToken) {
        this.pendingDeviceTokenRetry = false;
      }
    }
    const authToken = selectedAuth.authToken;
    console.log("[sendConnect] Final authToken:", authToken ? "present" : "missing");
  ...
  request<T = unknown>(method: string, params?: unknown): Promise<T> {
    ...
    const requestParams: Record<string, unknown> = { ...(params as Record<string, unknown> || {}) };
    const prefixesToCheck = ["cron.", "sessions.", "agents.", "auth.admin.", "chat."];
    const needsToken = prefixesToCheck.some(prefix => method.startsWith(prefix));
