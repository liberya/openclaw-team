...
import { setCurrentUserId, getUserSpecificSessionKey } from "./storage.ts";
import type {
  AgentsListResult,
  PresenceEntry,
  HealthSummary,
...
type GatewayHost = {
  ...
  password: string;
  email: string;
  currentUser: import("./gateway.ts").LoggedInUser | null;
...
export function connectGateway(host: GatewayHost) {
  ...
  const hasEmailLogin = host.email?.trim() && host.password?.trim();
  
  // 始终尝试从 localStorage 获取认证 token
  const storedAuthToken = localStorage.getItem("authToken");
  console.log("[connectGateway] storedAuthToken:", storedAuthToken ? "present" : "missing");
  console.log("[connectGateway] hasEmailLogin:", hasEmailLogin);
  console.log("[connectGateway] host.settings.token:", host.settings.token);

  const client = new GatewayBrowserClient({
    url: host.settings.gatewayUrl,
    // 优先使用 localStorage 中的认证 token
    token: storedAuthToken || (hasEmailLogin ? undefined : (host.settings.token.trim() ? host.settings.token : undefined)),
    password: hasEmailLogin ? undefined : (host.password.trim() ? host.password : undefined),
    email: hasEmailLogin ? host.email.trim() : undefined,
    clientName: "openclaw-control-ui",
    clientVersion,
    mode: "webchat",
    instanceId: host.clientInstanceId,
    onHello: (hello) => {
      console.log("[onHello] Received hello, currentUser before:", host.currentUser);
      console.log("[onHello] client.getUser():", client.getUser());
      ...
      // 如果 client.getUser() 为空但 localStorage 有用户信息，则恢复
      if (!client.getUser() && localStorage.getItem("currentUser")) {
        try {
          currentUser = JSON.parse(localStorage.getItem("currentUser") || "{}");
          host.currentUser = currentUser;
          console.log("[onHello] Restored user from localStorage:", currentUser);
        } catch (e) {
          console.error("[onHello] Failed to parse stored user:", e);
        }
      } else {
      ...
      // 如果有用户登录，设置 currentUserId 并更新 sessionKey
      if (currentUser?.id) {
        setCurrentUserId(currentUser.id);
        const agentId = host.sessionKey.split(':')[1] || "main";
        const newSessionKey = getUserSpecificSessionKey(agentId);
